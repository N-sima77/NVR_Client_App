...
1. SDKの説明
  ...
  以下の開発環境でC++ Classを用いたアプリケーションを開発する場合は、
  "DLL"を使用してください。
...
　　　　　　　　　　　（※但し、Connect()の引数に設定するUID等は、既存GetUIDプロパティで取得したUIDを使用すること）
                     ---追加プロパティ---
                      GetUIDEx
  ...
  変更内容     : [サポートする製品]
                 ...
                 - NXレコーダのHTTP/HTTPSダウンロード機能を追加しました
                   (ダウンロード対象ファイル：n3、MP4)
                     ---追加メソッド---
                      HttpDownload
                 - AlmOperationメソッドに、指定chでのイベント録画停止機能を追加しました
...
　　　　　　　　 - 連続して複数プロセスでライブを開始すると、一部ライブが
                   停止するという問題を修正しました。
　　　　　　　　 - アプリケーションの開発環境に、Microsoft Visual C++ 2012、
　　　　　　　　　 を追加しました。
...
　　　　　　　　 - ファイル再生時の音声(AAC)出力に対応しました。
                 - SetCameraPositionメソッド/GetCameraPositionメソッドの
　　　　　　　　　 引数focusの最小値を変更しました。
...
　　　　　　　　　　* 複数インスタンスで多画面表示を行う際に、音声のAAC は
  ...
  変更内容     : [PS-API]
                 ...
                 - 以下の不具合を修正しました。
                   ...
                   - C#で作成したアプリケーションからRecCtrlメソッドを
                     使用した場合に、異常終了することがある
                     不具合を修正しました。
                   - Search、SearchExで検索結果が(16の倍数+1)件の場合に
                     最後の1件が取得できない不具合の修正。

                 - PS-API Interface Specifications for DLL Englishの誤記を修正。
  ...
  変更内容     : [PS-API]
                 ...
                 - H.264に対応しました。
                 - FTPダウンロード機能を追加しました。
                 - NetworkDiskRecorderの圧縮画像データに対応しました。
                   (OnImage/DecodeImage)
  ...
  変更内容     : - 圧縮映像データ取得機能を追加しました。(PS-API)
                 - 検索条件を追加しました。(PS-API)
